...
def plot_pareto_front(
    ...
    plot = Scatter(
        title="Pareto Front ({})".format(group_name),
        labels=["Evaluation Time (Seconds)", "Macro F1"],
        tight_layout=True,
        legend=True,
    )
...
def plot_wins_by_configuration(tasks, config_names, output_dir="plots", figure_name="wins_by_config"):
    ...
    for task_idx, task in enumerate(tasks):
        ...
        # Only plot as many bars as there are win_ratios (fix shape mismatch)
        plt.bar(
            x_indices[:len(win_ratios)] + task_offset,
            win_ratios,
    ...
    # Only set as many ticks as there are configs (to avoid mismatch)
    plt.xticks(x_indices[:len(config_names)] + bar_width/2, config_names, rotation=45, ha='right', fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlim(-bar_width, x_indices[max(len(config_names)-1,0)] + bar_width + group_spacing)
    
    # Draw vertical lines to separate configuration groups, but only if enough x_base
    for i in range(1, min(categories, len(x_base))):
...
def build_experiment_plot_data(data_dict, color_map, marker_map, linestyle_map):
    ...
    if "baseline" in data_dict:
        ...
        for method, df in methods.items():
            alias_name = f"{alias_name_dict['baseline'].capitalize()} ({method.capitalize()})"
...
def main_combined():
    ...
    # Use DataLoader for robust config-based loading
    config_path = "/home/coder/autogoal/experiments/text_generation/configs/multi-objective/candidates.yaml"
    data_root = "/home/coder/autogoal/experiments/text_generation/data/experience_store"
    ...
    for dataset in datasets:
        ...
        for exp in plot_data:
            if exp.alias_name.lower() == "baseline (zeroshot)":
